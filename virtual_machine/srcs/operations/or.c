...
** all the arguments that are of type registry will be
** initialized with the actual value in the registry.
** This would mean we lose the registry number we have to save the output in.
** We call get_indirect_arg_idx_mod which will
** check if the arguments are of type indirect.
